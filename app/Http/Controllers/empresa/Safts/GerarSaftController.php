...
class GerarSaftController extends Component
{
    ...
    {
        $this->saft['dataInicio'] = NULL;
        $this->saft['dataFinal'] = NULL;
    }
    ...
    public function printSaft()
    {
        ...
        $messages = [
            'saft.dataInicio.required' => 'Informe a data inicial',
            'saft.dataFinal.required' => 'Informe a data final',
        ];
    ...
    public function gerarSaft($saft)
    {
        ...
        $StartDate = str_replace("T", " ", $saft['dataInicio']) . " 00:00";;
        $EndDate = str_replace("T", " ", $saft['dataFinal']) . " 00:59";
        ...
        $header->appendChild($dom->createElement('SoftwareValidationNumber', '0000/AGT/2022'));
        $header->appendChild($dom->createElement('ProductID', 'MUTUE_NEGOCIOS/MUTUE SOLUÇÕES TECNOLOGICAS INTELIGENTES LDA'));
        ...
        $TotalCreditRetificada = DB::table('facturas_original')->whereBetween(DB::raw('DATE(created_at)'), array($StartDate, $EndDate))
            ->where('empresa_id', Auth::user()->empresa_id)
            ->where('anulado', 1)
            ->where('retificado', 'Não')
            ->where(function ($query) {
                $query->orwhere('facturas_original.tipo_documento', 1)
                    ->orWhere('facturas_original.tipo_documento', 2);
            })
            ->sum('facturas_original.total_incidencia');
        ...
        if ($notaCreditos) {
            ...
            foreach ($notaCreditos as $key => $notaCredito) {
                ...
                foreach ($notaCredito->notaCreditoItems as $key => $Item) {
                    ...
                    $line->appendChild($dom->createElement('UnitPrice', number_format($Item->quantidade_produto != 0?$Item->incidencia_produto / $Item->quantidade_produto:0, 2, ".", "")));
        ...
        foreach ($facturas_proforma as $key => $facturaProforma) {
            ...
            foreach ($facturaProforma->facturas_items as $key => $ftProformaItem) {
                ...
                $line->appendChild($dom->createElement('Description', $facturaProforma->descricao ? $facturaProforma->descricao : 'FACTURA ' . $facturaProforma->numeracaoFactura));
