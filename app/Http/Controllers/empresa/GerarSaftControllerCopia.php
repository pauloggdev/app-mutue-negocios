...
class GerarSaftControllerCopia extends Controller
{
    ...
    public function gerarSaft(Request $request)
    {
        ...
        // $StartDate = date("Y-m-d", strtotime($request->dataInicio)) . " 00:00";
        // $EndDate = date("Y-m-d", strtotime($request->dataFinal)) . " 00:59";


        $StartDate = "2022-09-20 00:00";
        $EndDate = "2022-09-21 00:00";
        ...
        //Notas de crédito
        $notaCreditos = NotaCredito::with(['notaCreditoItems', 'factura', 'formaPagamento', 'notaCreditoItens.produto', 'tipoDocumento'])
            ->where('empresa_id', Auth::user()->empresa_id)
        ...
        if ($notaCreditos) {
            ...
            foreach ($notaCreditos as $key => $notaCredito) {
                ...
                $InvoiceNo = $notaCredito->numeracao_notaCredito;
                ...
                $InvoiceType = $notaCredito->tipoDocumento->sigla;
                ...
                $invoice->appendChild($dom->createElement('CustomerID', $CustomerID));
                //Criar Line de Invoice foreach
                foreach ($notaCredito->notaCreditoItens as $key => $Item) {
                    ...
                    $line->appendChild($dom->createElement('ProductDescription', $Item->descricao_produto));
                    $line->appendChild($dom->createElement('Quantity', number_format($Item->quantidade_produto, 1, ".", "")));
                    $line->appendChild($dom->createElement('UnitOfMeasure', ($Item->produto->unidade_medida ? $Item->produto->unidade_medida : "un")));
                    $line->appendChild($dom->createElement('UnitPrice', number_format($Item->incidencia_produto / $Item->quantidade_produto, 2, ".", "")));
                    $line->appendChild($dom->createElement('TaxPointDate', Carbon::parse($notaCredito->created_at)->format('Y-m-d')));
                    $Description = $notaCredito->descricao ? $notaCredito->descricao : 'FACTURA ANULADA OU RETIFICADA ' . $notaCredito->factura->numeracao_notaCredito;
                    $References = $dom->createElement('References');
                    $References->appendChild($dom->createElement('Reference', $notaCredito->factura->numeracao_factura));
                    ...
                    if ($Item->produto->taxa->taxa > 0) {
                        ...
                        $TaxExemptionCode = $Item->produto->motivo->codigo_motivo;
                    }
                    $TaxCode = $Item->produto->taxa->taxa ? "NOR" : ($TaxType == "NS" ? "NS" : "ISE");
                    $Description = $Item->produto->taxa->taxa ? "Taxa Normal" : "Isenta";
                    $TaxPercentage = $Item->produto->taxa->taxa;
                ...
                $invoice->appendChild($documentTotals);
                $payment = $dom->createElement('Payment');
                $PaymentMechanism = $factura->formaPagamento ? $factura->formaPagamento->sigla : "OU";
         ...
         */
        $recibos = Recibos::with(['empresa', 'cliente', 'formaPagamento'])->where('empresa_id', auth()->user()->empresa_id)
            ->whereBetween(DB::raw('DATE(created_at)'), array($StartDate, $EndDate))
            ->get();
        ...
        foreach ($recibos as $key => $recibo) {
            ...
            $PaymentMethod->appendChild($dom->createElement('PaymentMechanism', $recibo->formaPagamento->sigla));
            ...
            $SourceDocumentID->appendChild($dom->createElement('OriginatingON', $factura->numeracaoFactura));
            $SourceDocumentID->appendChild($dom->createElement('InvoiceDate', Carbon::parse($factura->created_at)->format('Y-m-d')));
