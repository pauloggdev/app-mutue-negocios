...
export default {
  ...
  methods: {
      ...
      if (
        ...
        try {
          ...
          if (response.status === 200 && response.data.status != 401) {
            ...
            let responseFactura = await axios.get(`${this.router}/facturacao/imprimir-factura/${response.data.factura.id}/${this.facturacao.tipoFolha}`);
    ...
    showModal(produto) {
      ...
      let percentagemDesconto = (produto.desconto_produto * 100) / (produto.preco_venda_produto * produto.quantidade_produto);
      let order = {
        produto_id: produto.produto_id,
        designacao: produto.designacao,
        descontoAnterior: percentagemDesconto,
        quantidade_produto: produto.quantidade_produto,
        retencao_produto: produto.retencao_produto || this.facturacao.retencao ? true : false,
      };
    ...
    editaDadosProdutoNoCarrinho(order) {
      ...
      if (prod != undefined) {
        ...
        if (this.verificarQtProdutoEstoque(produtoInfo, order.quantidade_produto)) {

          var totalDesconto = (order.desconto_produto ? order.desconto_produto :0) + (this.descontoTotal?this.descontoTotal:0) ;

          prod.quantidade_produto = order.quantidade_produto;

          prod.desconto_produto = (prod.preco_venda_produto * prod.quantidade_produto * totalDesconto) / 100;

          prod.total_preco_produto = this.total_preco_produto(prod,prod.quantidade_produto);

          prod.retencao_produto = (prod.preco_venda_produto * prod.quantidade_produto - prod.desconto_produto) * retencao;

          prod.incidencia_produto = prod.preco_venda_produto * prod.quantidade_produto - prod.desconto_produto;
          prod.iva_produto = (prod.taxa / 100) * prod.incidencia_produto;
          prod.valor_com_iva = prod.incidencia_produto + prod.iva_produto - prod.retencao_produto;
    ...
    adicionarProduto(produto) {
      ...
      if (prod != undefined) {
        ...
        if (this.verificarQtProdutoEstoque(produto, quant)) {
          ...
          prod.desconto_produto = this.valor_desconto(prod,prod.quantidade_produto);
          prod.total_preco_produto = this.total_preco_produto(prod, prod.quantidade_produto);
    ...
    setarValorTotalFacturacao() {
      ...
      this.facturacao.produtos.map((item, index) => {
        this.facturacao.desconto += this.facturacao.produtos[index].desconto_produto;
        this.facturacao.numeroItems += this.facturacao.produtos[index].quantidade_produto;
        this.facturacao.total_retencao += this.facturacao.produtos[index].retencao_produto;
        this.facturacao.total_incidencia += this.facturacao.produtos[index].incidencia_produto;
        this.facturacao.total_iva += this.facturacao.produtos[index].iva_produto;
        this.facturacao.total_preco_factura += this.facturacao.produtos[index].total_preco_produto;
      });
    ...
    valor_desconto(produto, quantidade) {
      ...
      var descontoItem = produto.produto_desconto == undefined ? 0: produto.produto_desconto;
      var descontoGeral = this.descontoTotal == undefined ? 0: this.descontoTotal;
