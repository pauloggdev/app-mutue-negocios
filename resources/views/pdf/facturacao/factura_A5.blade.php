...
<html>
    <head>
        ...
        <title>{{$factura->tipo_documento}} | A5</title>
    ...
    <body onload="window.print()";>
        ...
        <div class="row" style="font-size: 7pt;border-bottom: 0.3px solid #403e3eb0!important; padding:5px;"></div>
            ...
            <div style="font-size: 7pt;">
                <table class="table table-striped" >
                    <tr>
                        ...
                        <th>{{$factura->numeracaoFactura}}</th>
                        @elseif($factura->tipo_documento==3)
                        <th>{{$factura->numeracaoFactura}}</th>
                        @elseif($factura->tipo_documento==1)
                        <th>{{$factura->numeracaoFactura}}</th>
                        @endif
                        <th>Moeda: {{mb_strtoupper($factura->moeda)}}</th>
                        <th>Câmbio: {{number_format($factura->cambio,2,",",".")}}</td></th>
                        @if($factura->formato_documento == 2 || $factura->formato_documento ==1)
                        <th>Forma de Pagamento: {{mb_strtoupper($factura->forma_pagamento)}}</th>
                        @endif
                    </tr>
                    <tr>
                        <td></td>
        ...
        <div class="row">
            <div style="font-size: 7pt;">
                <table class="table table-striped">
                    ...
                    <tr>
                        <td>{{++$key}}</td>
                        <td style="text-align: left;">{{mb_strtoupper($item->descricao_produto)}}</td>
                        <td style="text-align: right;">{{number_format($item->preco_venda_produto,2,",",".")}}</td>
                        <td>{{number_format($item->quantidade_produto,2,",",".")}}</td>
                        <td>{{$item->unidade }}</td>
                        <td>{{number_format(($item->desconto_produto/$item->total_preco_produto)*100,1,",",".")}}%</td>
                        <td>{{number_format($item->valorTaxa,1,",",".")}}%</td>
        ...
        <div class="row">
            <div class="pull-left" style="font-size: 8pt; width:60%">
                <table class="table table-striped">
                  ...
                  <tbody>
                    @foreach ($facturaTaxa as $itemTaxa)
                        <tr>
                            <td>{{$itemTaxa->descricao}}</td>
                            <td style="text-align: right;">{{number_format($itemTaxa->total_incidencia,2,",",".")}}</td>
                            <td style="text-align: right;">{{number_format($itemTaxa->total_iva,2,",",".")}}</td>
                            <td>{{$itemTaxa->motivoDescricao - $itemTaxa->codigoMotivo}}</td>
                        </tr>
                    @endforeach
                  </tbody>
                </table>
        ...
        <br>
        @endfor
        <div class="row" style="font-size: 7pt;border: 1px solid black!important; padding:10px;"> 
        ...
        <div class="row">
            <div class="pull-left">
                <ul>
                    ...
                    <li style="font-weight: bolder;">{{mb_strtoupper(substr($factura->hashValor, 0, 4))}}-Processado por programa Certificado nº X/AGT/2020 (Mutue-Negócios)</li>
                </ul>
            </div>
        </div>
        
        <div class="row" style="font-size: 7pt;border-bottom: 1px solid black!important; padding:5px;"></div>
        <div class="row" style="font-size: 7pt;">
